...
private:
  ...
  const char exitMarker, entryMarker, visited, passage, wall, vert, horiz;
  ...
  void const setDirectional(Stack<Cell>);
};
...
//
///////////////////////////////////////////////////////////////////////////////
Maze::Maze() : exitMarker('e'), entryMarker('m'), visited('.'), passage('0'), wall('1'), vert('|'), horiz('-') {
  Stack<char*> mazeRows;
  char str[80], *s;
  int col, row = 0;
...
void Maze::exitMaze() {
  ...
  cout << endl << "Final maze with path: " << endl;
  setDirectional(tmpStack);
  cout << *this;
...
///////////////////////////////////////////////////////////////////////////////
//
//  Function: void const Maze::setDirectional(Stack<Cell> tmpStack)
//
//  Description: This function loops through the stack that contains the end
//  path without dead ends and compares the coordinates in neighbouring cells
//  to determine if the path is moving horizontally or vertically.  Depending
//  on which way it is moving, it will set the marker for that cell to be a -
//  for a horizontal move or a | for a vertical move.
//
//  Preconditions: A stack with the end path must be passed to the function.
//
//  Postconditions: The store that holds the markers for the maze will be adjusted
//  so that the path the mouse takes will be defined with a line showing direction.
//
///////////////////////////////////////////////////////////////////////////////
void const Maze::setDirectional(Stack<Cell> tmpStack) {
	Stack<Cell> dStack;
	Cell tmp1, tmp2;		
	tmp1 = tmpStack.pop();
	while (!(tmpStack.empty())) {
		tmp2 = tmpStack.pop();
		if (tmp1.x != tmp2.x) {
			if (store[tmp1.x][tmp1.y] == exitMarker || store[tmp1.x][tmp1.y] == entryMarker) { }
			else { store[tmp1.x][tmp1.y] = vert; }
		} else {
			if (store[tmp1.x][tmp1.y] == exitMarker || store[tmp1.x][tmp1.y] == entryMarker) { }
			else { store[tmp1.x][tmp1.y] = horiz; }
		}
		dStack.push(tmp1);
		dStack.push(tmp2);
		tmp1 = tmp2;
